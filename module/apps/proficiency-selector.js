...
export default class ProficiencySelector extends TraitSelector {
  ...
  static async getChoices(type, chosen=[]) {
    let data = Object.entries(CONFIG.CNC[`${type}Proficiencies`]).reduce((obj, [key, label]) => {
      obj[key] = { label: label, chosen: chosen.includes(key) };
      return obj;
    }, {});

    const ids = CONFIG.CNC[`${type}Ids`];
    const map = CONFIG.CNC[`${type}ProficienciesMap`];
    if ( ids !== undefined ) {
    ...
    if ( type === "tool" ) {
      data.vehicle.children = Object.entries(CONFIG.CNC.vehicleTypes).reduce((obj, [key, label]) => {
        obj[key] = { label: label, chosen: chosen.includes(key) };
        return obj;
      }, {});
      data = game.cnc.utils.sortObjectEntries(data, "label");
    }

    for ( const category of Object.values(data) ) {
      if ( !category.children ) continue;
      category.children = game.cnc.utils.sortObjectEntries(category.children, "label");
    }
   ...
   *                                       false.
   * @returns {Promise<ItemCnc>|object}     Promise for a `Document` if `indexOnly` is false & `fullItem` is true,
  ...
  static getBaseItem(identifier, { indexOnly=false, fullItem=false }={}) {
    let pack = CONFIG.CNC.sourcePacks.ITEMS;
    ...
    // Full ItemCnc document required, always async.
