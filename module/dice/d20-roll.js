/**
 * A type of Roll specific to a d20-based check, save, or attack roll in the CNC system.
...
export default class D20Roll extends Roll {
  ...
  /**
   * Advantage mode of a CNC d20 roll
   * @enum {number}
   */
  static ADV_MODE = {
    NORMAL: 0,
   ...
   * @type {string}
   */
  static EVALUATION_TEMPLATE = "systems/cnc/templates/chat/roll-dialog.html";
  ...
  async toMessage(messageData={}, options={}) {
    ...
    // Add appropriate advantage mode message flavor and cnc roll flags
    messageData.flavor = messageData.flavor || this.options.flavor;
    if ( this.hasAdvantage ) messageData.flavor += ` (${game.i18n.localize("CNC.Advantage")})`;
    else if ( this.hasDisadvantage ) messageData.flavor += ` (${game.i18n.localize("CNC.Disadvantage")})`;

    // Add reliable talent to the d20-term flavor text if it applied
    if ( this.options.reliableTalent ) {
      const d20 = this.dice[0];
      const isRT = d20.results.every(r => !r.active || (r.result < 10));
      const label = `(${game.i18n.localize("CNC.FlagsReliableTalent")})`;
  ...
  async configureDialog({title, defaultRollMode, defaultAction=D20Roll.ADV_MODE.NORMAL, chooseModifier=false,
    ...
    const content = await renderTemplate(template ?? this.constructor.EVALUATION_TEMPLATE, {
      ...
      chooseModifier,
      defaultAbility,
      abilities: CONFIG.CNC.abilities
    });
    ...
    return new Promise(resolve => {
      new Dialog({
        ...
        buttons: {
          advantage: {
            label: game.i18n.localize("CNC.Advantage"),
            callback: html => resolve(this._onDialogSubmit(html, D20Roll.ADV_MODE.ADVANTAGE))
          },
          normal: {
            label: game.i18n.localize("CNC.Normal"),
            callback: html => resolve(this._onDialogSubmit(html, D20Roll.ADV_MODE.NORMAL))
          },
          disadvantage: {
            label: game.i18n.localize("CNC.Disadvantage"),
  ...
  _onDialogSubmit(html, advantageMode) {
    ...
    if ( form.ability?.value ) {
      ...
      this.options.flavor += ` (${CONFIG.CNC.abilities[form.ability.value]})`;
    }
