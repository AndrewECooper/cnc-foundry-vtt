...
export default class AbilityUseDialog extends Dialog {
   ...
   * @param {ItemCnc} item  Item being used.
  ...
  static async create(item) {
    ...
    const data = {
      item: item.data,
      title: game.i18n.format("CNC.AbilityUseHint", {type: game.i18n.localize(`CNC.ItemType${item.type.capitalize()}`), name: item.name}),
    ...
    return new Promise(resolve => {
      const dlg = new this(item, {
        title: `${item.name}: ${game.i18n.localize("CNC.AbilityUseConfig")}`,
        content: html,
        buttons: {
          use: {
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    if ( !canCast ) data.errors.push(game.i18n.format("CNC.SpellCastNoSlots", {
      level: CONFIG.CNC.spellLevels[lvl],
      name: data.item.name
    }));
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if ( recharge.value ) {
      return game.i18n.format(recharge.charged ? "CNC.AbilityUseChargedHint" : "CNC.AbilityUseRechargeHint", {
        type: game.i18n.localize(`CNC.ItemType${item.type.capitalize()}`)
      });
    }
    ...
    if ( item.type === "consumable" ) {
      ...
      return game.i18n.format(str, {
        type: game.i18n.localize(`CNC.Consumable${item.data.consumableType.capitalize()}`),
        value: uses.value,
