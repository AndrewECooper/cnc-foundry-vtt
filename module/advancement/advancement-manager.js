 ...
 * @property {ItemCnc} [item]              For "delete" steps only, the item to be removed.
 * @property {object} [class]             Contains data on class if step was triggered by class level change.
 * @property {ItemCnc} [class.item]        Class item that caused this advancement step.
 ...
 *
 * @property {ActorCnc} actor        Actor on which this advancement is being performed.
...
export class AdvancementManager extends Application {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["cnc", "advancement", "flow"],
      template: "systems/cnc/templates/advancement/advancement-manager.html",
      width: 460,
      height: "auto"
    });
  }
  ...
  get title() {
    ...
    const step = visibleIndex < 0 ? "" : game.i18n.format("CNC.AdvancementManagerSteps", {
      current: visibleIndex + 1,
      total: visibleSteps.length
    });
    return `${game.i18n.localize("CNC.AdvancementManagerTitle")} ${step}`;
  }
   ...
   * @param {ActorCnc} actor         Actor to which the item is being added.
   ...
   * @param {ActorCnc} actor         Actor from which the choices should be modified.
   ...
   * @param {ActorCnc} actor         Actor from which the item should be deleted.
   ...
   * @param {ActorCnc} actor         Actor whose level has changed.
   ...
   * @param {ItemCnc} item          Item that has advancement.
  ...
  getData() {
    ...
    return {
      ...
      header: item.name,
      subheader: level ? game.i18n.format("CNC.AdvancementLevelHeader", { level }) : "",
      steps: {
        current: visibleIndex + 1,
        total: visibleSteps.length,
  ...
  render(...args) {
     ...
     * @function cnc.preAdvancementManagerRender
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager about to be rendered
     */
    const allowed = Hooks.call("cnc.preAdvancementManagerRender", this);
  ...
  async close(options={}) {
    if ( !options.skipConfirmation ) {
      return new Dialog({
        title: `${game.i18n.localize("CNC.AdvancementManagerCloseTitle")}: ${this.actor.name}`,
        content: game.i18n.localize("CNC.AdvancementManagerCloseMessage"),
        buttons: {
          close: {
            icon: '<i class="fas fa-times"></i>',
            label: game.i18n.localize("CNC.AdvancementManagerCloseButtonStop"),
            callback: () => super.close(options)
          },
          continue: {
            icon: '<i class="fas fa-chevron-right"></i>',
            label: game.i18n.localize("CNC.AdvancementManagerCloseButtonContinue")
          }
        },
        default: "close"
      }).render(true);
  ...
  async _restart(event) {
    const restart = await Dialog.confirm({
      title: game.i18n.localize("CNC.AdvancementManagerRestartConfirmTitle"),
      content: game.i18n.localize("CNC.AdvancementManagerRestartConfirm")
    });
    if ( !restart ) return;
  ...
  async _complete(event) {
     ...
     * are applied.
     * @function cnc.preAdvancementManagerComplete
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("cnc.preAdvancementManagerComplete", this, updates, toCreate, toUpdate, toDelete) === false ) {
     ...
     * @function cnc.advancementManagerComplete
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager that just completed
     */
    Hooks.callAll("cnc.advancementManagerComplete", this);
