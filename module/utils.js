...
export function _linkForUuid(uuid) {
  const index = game.cnc.utils.indexFromUuid(uuid);

  let link;

  if ( !index ) {
    link = `@Item[${uuid}]{${game.i18n.localize("CNC.Unknown")}}`;
 ...
 * @param {string} configKey              Key within `CONFIG.CNC` to localize.
 ...
 * @param {object} config  The `CONFIG.CNC` object to localize and sort. *Will be mutated.*
 */
