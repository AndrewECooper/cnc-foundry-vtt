...
export default class ActorSheetCNCCharacter extends ActorSheetCnC {
  ...
  getData() {
    ...
    // Weight unit
    sheetData.weightUnit = game.settings.get("cnc", "metricWeightUnits")
      ? game.i18n.localize("CNC.AbbreviationKgs")
      : game.i18n.localize("CNC.AbbreviationLbs");
  ...
  _prepareItems(data) {
    ...
    let {items, spells, feats, backgrounds, classes, subclasses} = data.items.reduce((obj, item) => {
      ...
      item.attunement = {
        [CONFIG.CNC.attunementTypes.REQUIRED]: {
          icon: "fa-sun",
          cls: "not-attuned",
          title: "CNC.AttunementRequired"
        },
        [CONFIG.CNC.attunementTypes.ATTUNED]: {
          icon: "fa-sun",
          cls: "attuned",
    ...
    const maxLevelDelta = CONFIG.CNC.maxLevel - this.actor.data.data.details.level;
    classes = classes.reduce((arr, cls) => {
      cls.availableLevels = Array.fromRange(CONFIG.CNC.maxLevel + 1).slice(1).map(level => {
    ...
    for ( const subclass of subclasses ) {
      feats.push(subclass);
      this.actor._preparationWarnings.push(game.i18n.format("CNC.SubclassMismatchWarn", {
    ...
    const features = {
      background: {
        label: "CNC.ItemTypeBackground", items: backgrounds,
        hasActions: false, dataset: {type: "background"} },
      classes: {
        label: "CNC.ItemTypeClassPl", items: classes,
        hasActions: false, dataset: {type: "class"}, isClass: true },
      active: {
        label: "CNC.FeatureActive", items: [],
        hasActions: true, dataset: {type: "feat", "activation.type": "action"} },
      passive: {
        label: "CNC.FeaturePassive", items: [],
   ...
   * @param {ItemCnc} item  Item being prepared for display. *Will be mutated.*
   * @private
   */
   ...
   * @returns {Promise<AdvancementManager|ItemCnc>}  Manager if advancements needed, otherwise updated class item.
   * @private
   */
  async _onLevelChange(event) {
    ...
    if ( classItem.hasAdvancement && !game.settings.get("cnc", "disableAdvancements") ) {
   ...
   * @returns {Promise<ItemCnc>}  Item with the updates applied.
   * @private
   */
  _onToggleItem(event) {
    event.preventDefault();
  ...
  async _onDropItemCreate(itemData) {
    ...
    if ( itemData.type === "class" ) {
      itemData.data.levels = Math.min(itemData.data.levels,
        CONFIG.CNC.maxLevel - this.actor.data.data.details.level);
      if ( itemData.data.levels <= 0 ) return ui.notifications.error(
        game.i18n.format("CNC.MaxCharacterLevelExceededWarn", {max: CONFIG.CNC.maxLevel})
      );

      const cls = this.actor.itemTypes.class.find(c => c.identifier === itemData.data.identifier);
      if ( cls ) {
        const priorLevel = cls.data.data.levels;
        if ( cls.hasAdvancement && !game.settings.get("cnc", "disableAdvancements") ) {
    ...
    // If a subclass is dropped, ensure it doesn't match another subclass with the same identifier
      ...
      if ( other ) {
        return ui.notifications.error(game.i18n.format("CNC.SubclassDuplicateError", {
          identifier: other.identifier
        }));
      }
      const cls = this.actor.itemTypes.class.find(i => i.identifier === itemData.data.classIdentifier);
      if ( cls && cls.subclass ) {
        return ui.notifications.error(game.i18n.format("CNC.SubclassAssignmentError", {
