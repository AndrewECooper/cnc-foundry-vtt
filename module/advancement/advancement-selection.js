...
export class AdvancementSelection extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["cnc", "sheet", "advancement"],
      template: "systems/cnc/templates/advancement/advancement-selection.html",
      title: "CNC.AdvancementSelectionTitle",
      width: 500,
      height: "auto"
    });
  }
  ...
  getData() {
    ...
    for ( const advancement of Object.values(game.cnc.advancement.types) ) {
    ...
    data.types = game.cnc.utils.sortObjectEntries(data.types, "label");
   ...
   * been resolved.
   * @param {ItemCnc} item                         Item to which the advancement should be added.
   * @param {object} [config={}]
   ...
   * @returns {Promise<AdvancementConfig|null>}   Result of `ItemCnc#createAdvancement`.
   */
  static async createDialog(item, { rejectClose=false, options={} }={}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(item, {
        title: `${game.i18n.localize("CNC.AdvancementSelectionTitle")}: ${item.name}`,
        buttons: {
          submit: {
            callback: html => {
