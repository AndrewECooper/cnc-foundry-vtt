...
import ItemCNC from "../item/entity.js";
...
export default class ActorCnc extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("cnc", "isPolymorphed") || false;
  }
  ...
  prepareDerivedData() {
    ...
    const flags = actorData.flags.cnc || {};
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag("cnc", "transformOptions");
      const original = game.actors?.get(this.getFlag("cnc", "originalActor"));
      if (original) {
    ...
    if ( this.type !== "vehicle" ) data.attributes.attunement.value = this.items.filter(i => {
      return i.data.data.attunement === CONFIG.CNC.attunementTypes.ATTUNED;
    }).length;
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.CNC.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.CNC.CR_EXP_LEVELS[cr];
  }
   ...
   * @param {ItemCNC[]} items         The items being added to the Actor.
   * @param {boolean} [prompt=true]  Whether or not to prompt the user.
   * @returns {Promise<ItemCNC[]>}
   * @deprecated since cnc 1.6, targeted for removal in 1.8
   */
  async addEmbeddedItems(items, prompt=true) {
    console.warn("ActorCnc#addEmbeddedItems has been deprecated and will be removed in 1.8.");
    ...
    if (prompt) {
      const itemIdsToAdd = await SelectItemsPrompt.create(items, {
        hint: game.i18n.localize("CNC.AddEmbeddedItemPromptHint")
      });
    ...
    return ItemCNC.createDocuments(toCreate, {parent: this});
  }
   ...
   * @returns {Promise<ItemCNC[]>}              Any new items that should be added to the actor.
   * @deprecated since cnc 1.6, targeted for removal in 1.8
   */
  async getClassFeatures({classIdentifier, subclassName, level}={}) {
    console.warn("ActorCnc#getClassFeatures has been deprecated and will be removed in 1.8. Please refer to the Advancement API for its replacement.");
    const existing = new Set(this.items.map(i => i.name));
    const features = await ActorCnc.loadClassFeatures({classIdentifier, subclassName, level});
   ...
   * @returns {Promise<ItemCNC[]>}              Items that should be added based on the changes made.
   * @deprecated since cnc 1.6, targeted for removal in 1.8
   */
  static async loadClassFeatures({classIdentifier="", subclassName="", level=1, priorLevel=0}={}) {
    console.warn("ActorCnc#loadClassFeatures has been deprecated and will be removed in 1.8. Please refer to the Advancement API for its replacement.");
    subclassName = subclassName.slugify();

    // Get the configuration of features which may be added
    const clsConfig = CONFIG.CNC.classFeatures[classIdentifier];
    if (!clsConfig) return [];
  ...
  /**
   * Update the actor's abilities list to match the abilities configured in `CNC.abilities`.
  ...
  _prepareBaseAbilities(actorData, updates) {
    ...
    for ( const key of Object.keys(CONFIG.CNC.abilities) ) {
  ...
  _prepareSkills(actorData, bonusData, bonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = CONFIG.CNC.characterFlags;
    const joat = flags.jackOfAllTrades;
  ...
  _computeSpellcastingProgression(actorData) {
    ...
    const levels = Math.clamped(progression.slot, 0, CONFIG.CNC.maxLevel);
    const slots = CONFIG.CNC.SPELL_SLOT_TABLE[Math.min(levels, CONFIG.CNC.SPELL_SLOT_TABLE.length) - 1] || [];
    ...
    let pl = Math.clamped(progression.pact, 0, CONFIG.CNC.maxLevel);
   ...
   *   cover: number,
   *   flat: number,
   *   equippedArmor: ItemCNC,
   *   equippedShield: ItemCNC,
   *   warnings: string[]
   * }}
   * @private
   */
  _computeArmorClass(data) {
    ...
    ac.warnings = [];
    let cfg = CONFIG.CNC.armorClasses[ac.calc];
    if ( !cfg ) {
      ac.calc = "flat";
      if ( Number.isNumeric(ac.value) ) ac.flat = Number(ac.value);
      cfg = CONFIG.CNC.armorClasses.flat;
    }

    // Identify Equipped Items
    const armorTypes = new Set(Object.keys(CONFIG.CNC.armorTypes));
    ...
    switch ( ac.calc ) {
      ...
      default:
        ...
        if ( armors.length ) {
          if ( armors.length > 1 ) ac.warnings.push("CNC.WarnMultipleArmor");
        ...
        try {
          ...
          ac.warnings.push("CNC.WarnBadACFormula");
          const replaced = Roll.replaceFormulaData(CONFIG.CNC.armorClasses.default.formula, rollData);
          ac.base = Roll.safeEval(replaced);
        }
        break;
    }
    ...
    // Equipped Shield
    if ( shields.length ) {
      if ( shields.length > 1 ) ac.warnings.push("CNC.WarnMultipleShields");
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("cnc", "currencyWeight") && actorData.data.currency ) {
      const currency = actorData.data.currency;
      const numCoins = Object.values(currency).reduce((val, denom) => val += Math.max(denom, 0), 0);

      const currencyPerWeight = game.settings.get("cnc", "metricWeightUnits")
        ? CONFIG.CNC.encumbrance.currencyPerWeight.metric
        : CONFIG.CNC.encumbrance.currencyPerWeight.imperial;
    ...
    const strengthMultiplier = game.settings.get("cnc", "metricWeightUnits")
      ? CONFIG.CNC.encumbrance.strMultiplier.metric
      : CONFIG.CNC.encumbrance.strMultiplier.imperial;
  ...
  async _preCreate(data, options, user) {
    ...
    // Token size category
    const s = CONFIG.CNC.tokenSizes[this.data.data.traits.size || "med"];
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( newSize && (newSize !== foundry.utils.getProperty(this.data, "data.traits.size")) ) {
      let size = CONFIG.CNC.tokenSizes[newSize];
   ...
   * @returns {Promise<ActorCnc>}  Instance of the updated actor.
   * @protected
   */
  _assignPrimaryClass() {
   ...
   * @returns {Promise<ActorCnc>}  A Promise which resolves once the damage has been applied
   */
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("cnc", "remarkableAthlete")
      && CONFIG.CNC.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("cnc", "reliableTalent"));

    // Roll and return
    const flavor = game.i18n.format("CNC.SkillPromptTitle", {skill: CONFIG.CNC.skills[skillId]});
    const rollData = foundry.utils.mergeObject(options, {
      ...
      flavor,
      chooseModifier: true,
      halflingLucky: this.getFlag("cnc", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.cnc.roll": {type: "skill", skillId }
      }
    });
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.CNC.abilities[abilityId] ?? "";
    new Dialog({
      title: `${game.i18n.format("CNC.AbilityPromptTitle", {ability: label})}: ${this.name}`,
      content: `<p>${game.i18n.format("CNC.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("CNC.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("CNC.ActionSave"),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("CNC.AbilityPromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject(options, {
      parts,
      data,
      title: `${flavor}: ${this.name}`,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.cnc.roll": {type: "ability", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("CNC.SavePromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject(options, {
      parts,
      data,
      title: `${flavor}: ${this.name}`,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.cnc.roll": {type: "save", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.data.data.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3)) {
      ui.notifications.warn(game.i18n.localize("CNC.DeathSaveUnnecessary"));
      return null;
    }
    ...
    if ( this.getFlag("cnc", "diamondSoul") ) {
      parts.push("@prof");
    ...
    // Evaluate the roll
    const flavor = game.i18n.localize("CNC.DeathSavingThrow");
    const rollData = foundry.utils.mergeObject(options, {
      parts,
      data,
      title: `${flavor}: ${this.name}`,
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        chatString = "CNC.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        chatString = "CNC.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        chatString = "CNC.DeathSaveFailure";
      }
    }
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    if ( !cls ) {
      ui.notifications.error(game.i18n.format("CNC.HitDiceWarn", {name: this.name, formula: denomination}));
      return null;
    }

    // Prepare roll data
    const parts = [`1${denomination}`, "@abilities.con.mod"];
    const flavor = game.i18n.localize("CNC.HitDiceRoll");
   ...
   * @param {ItemCNC} item      The class item whose hit dice to roll.
   * @returns {Promise<Roll>}  The completed roll.
   * @see {@link cnc.preRollClassHitPoints}
   */
  async rollClassHitPoints(item) {
    ...
    const flavor = game.i18n.format("CNC.AdvancementHitPointsRollMessage", { class: item.name });
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      "flags.cnc.roll": { type: "hitPoints" }
    };

    /**
     * A hook event that fires before hit points are rolled for a character's class.
     * @function cnc.preRollClassHitPoints
     * @memberof hookEvents
     * @param {ActorCnc} actor            Actor for which the hit points are being rolled.
     * @param {ItemCNC} item              The class item whose hit dice will be rolled.
     ...
     */
    Hooks.callAll("cnc.preRollClassHitPoints", this, item, rollData, messageData);
  ...
  async _rest(chat, newDay, longRest, dhd=0, dhp=0) {
    ...
    if ( Hooks._hooks.restCompleted?.length ) console.warn(
      "The restCompleted hook has been deprecated in favor of cnc.restCompleted. "
      + "The original hook will be removed in cnc 1.8."
    );
     ...
     * @function cnc.restCompleted
     * @memberof hookEvents
     * @param {ActorCnc} actor      The actor that just completed resting.
     * @param {RestResult} result  Details on the rest completed.
     */
    Hooks.callAll("cnc.restCompleted", this, result);
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    switch (game.settings.get("cnc", "restVariant")) {
      case "normal": restFlavor = (longRest && newDay) ? "CNC.LongRestOvernight" : `CNC.${length}RestNormal`; break;
      case "gritty": restFlavor = (!longRest && newDay) ? "CNC.ShortRestOvernight" : `CNC.${length}RestGritty`; break;
      case "epic": restFlavor = `CNC.${length}RestEpic`; break;
    }

    // Determine the chat message to display
    if ( diceRestored && healthRestored ) message = `CNC.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "CNC.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "CNC.LongRestResultHitDice";
    else message = `CNC.${length}RestResultShort`;
   ...
   * carried by an Actor.
   * @returns {Promise<ActorCnc>}
   */
  convertCurrency() {
    const curr = foundry.utils.deepClone(this.data.data.currency);
    const conversion = Object.entries(CONFIG.CNC.currencies);
    conversion.reverse();
   ...
   *
   * @param {ActorCnc} target                      The target Actor.
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("cnc", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("CNC.PolymorphWarn"));
    }

    // Get the original Actor data and the new source data
    const o = this.toJSON();
    o.flags.cnc = o.flags.cnc || {};
    o.flags.cnc.transformOptions = {mergeSkills, mergeSaves};
    ...
    if ( !keepClass && d.data.details.cr ) {
      d.items.push({
        type: "class",
        name: game.i18n.localize("CNC.PolymorphTmpClass"),
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.cnc.originalActor ) d.flags.cnc.originalActor = this.id;
    d.flags.cnc.isPolymorphed = true;
     ...
     * @function cnc.transformActor
     * @memberof hookEvents
     * @param {ActorCnc} actor                  The original actor before transformation.
     * @param {ActorCnc} target                 The target actor into which to transform.
     * @param {object} data                    The data that will be used to create the new transformed actor.
     * @param {TransformationOptions} options  Options that determine how the transformation is performed.
     */
    Hooks.callAll("cnc.transformActor", this, target, d, {
  ...
  async revertOriginalForm() {
    ...
    if ( !this.isOwner ) {
      return ui.notifications.warn(game.i18n.localize("CNC.PolymorphRevertWarn"));
    }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: "CNC.PolymorphRestoreTransformation",
  ...
  static formatCreatureType(typeData) {
    ...
    if ( typeData.value === "custom" ) {
      ...
      let code = CONFIG.CNC.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? `${code}Pl` : code);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("CNC.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.CNC.actorSizes[typeData.swarm]),
        type: localizedType
      });
    }
  ...
  static prepareProficiencies(data, type) {
    const profs = CONFIG.CNC[`${type}Proficiencies`];
    const itemTypes = CONFIG.CNC[`${type}Ids`];
    ...
    for ( const key of values ) {
      ...
      } else if ( type === "tool" && CONFIG.CNC.vehicleTypes[key] ) {
        data.selected[key] = CONFIG.CNC.vehicleTypes[key];
      }
    }
  ...
  _displayScrollingDamage(dhp) {
    ...
    for ( let t of tokens ) {
      ...
      t.hud.createScrollingText(dhp.signedString(), {
        ...
        fill: CONFIG.CNC.tokenHPColors[dhp < 0 ? "damage" : "healing"],
        stroke: 0x000000,
        strokeThickness: 4,
        jitter: 0.25
      });
   ...
   * @param {string} ability  Ability key as defined in `CONFIG.CNC.abilities`.
   * @returns {number}        Spell save DC for provided ability.
   * @deprecated since cnc 0.97
   */
  getSpellDC(ability) {
    console.warn("The ActorCnc#getSpellDC(ability) method has been deprecated in favor of ActorCnc#data.data.abilities[ability].dc");
  ...
  async useSpell(item, {configureDialog=true}={}) {
    console.warn("The ActorCnc#useSpell method has been deprecated in favor of ItemCNC#roll");
