import ActorCnc from "../actor/entity.js";
...
export default class ActorTypeConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["cnc", "actor-type", "trait-selector"],
      template: "systems/cnc/templates/apps/actor-type.html",
      width: 280,
      height: "auto",
      choices: {},
      allowCustom: true,
  ...
  /** @inheritdoc */
  get title() {
    return `${game.i18n.localize("CNC.CreatureTypeTitle")}: ${this.object.name}`;
  }
  ...
  getData(options) {
    ...
    if ( foundry.utils.getType(attr) !== "Object" ) attr = {
      value: (attr in CONFIG.CNC.creatureTypes) ? attr : "humanoid",
      subtype: "",
      swarm: "",
      custom: ""
    };
    ...
    // Populate choices
    const types = {};
    for ( let [k, v] of Object.entries(CONFIG.CNC.creatureTypes) ) {
      types[k] = {
    ...
    return {
      ...
      custom: {
        value: attr.custom,
        label: game.i18n.localize("CNC.CreatureTypeSelectorCustom"),
        chosen: attr.value === "custom"
      },
      subtype: attr.subtype,
      swarm: attr.swarm,
      sizes: Array.from(Object.entries(CONFIG.CNC.actorSizes)).reverse().reduce((obj, e) => {
        obj[e[0]] = e[1];
        return obj;
      }, {}),
      preview: ActorCnc.formatCreatureType(attr) || "–"
    };
  }
  ...
  _onChangeInput(event) {
    ...
    this.form.preview.value = ActorCnc.formatCreatureType(typeObject) || "—";
  }
