 ...
 * @extends {Item}
 */
export default class ItemCnc extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.CNC.areaTargetTypes);
  }
   ...
   */
  get isArmor() {
    return this.data.data.armor?.type in CONFIG.CNC.armorTypes;
  }
  ...
  get areEffectsSuppressed() {
    ...
    return this.data.data.attunement === CONFIG.CNC.attunementTypes.REQUIRED;
  }
  ...
  prepareDerivedData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("CNC.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("CNC.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "CNC.Attack" : "CNC.Action");
      else labels.featType = game.i18n.localize("CNC.Passive");
    }

    // Equipment Items
    else if ( itemData.type === "equipment" ) {
      labels.armor = data.armor.value ? `${data.armor.value} ${game.i18n.localize("CNC.AC")}` : "";
    }
    ...
    if ( data.hasOwnProperty("activation") ) {
      ...
      labels.recharge = `${game.i18n.localize("CNC.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
  ...
  _prepareAdvancement() {
    ...
    this.advancement = {
      ...
      byLevel: Object.fromEntries(
        Array.fromRange(CONFIG.CNC.maxLevel + 1).slice(minAdvancementLevel).map(l => [l, []])
      ),
      byType: {},
      needingConfiguration: []
    };
    for ( const advancementData of this.data.data.advancement ?? [] ) {
      const Advancement = game.cnc.advancement.types[`${advancementData.type}Advancement`];
   ...
   * will be called at the end of `ItemCnc#prepareDerivedData`.
   */
  prepareFinalAttributes() {
    ...
    if ( this.data.data.hasOwnProperty("actionType") ) {
      // Ability checks
      this.labels.abilityCheck = game.i18n.format("CNC.AbilityPromptTitle", {
        ability: CONFIG.CNC.abilities[this.data.data?.ability]
      });
   ...
   * actor data. This is only used for display purposes and is not related to `ItemCnc#rollDamage`.
  ...
  getDerivedDamageLabel() {
    ...
    const damageLabels = { ...CONFIG.CNC.damageTypes, ...CONFIG.CNC.healingTypes };
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.CNC.abilities[save.ability] ?? "";
    this.labels.save = game.i18n.format("CNC.SaveDC", {dc: save.dc || "", ability: abl});
    return save.dc;
  }
  ...
  getCriticalThreshold() {
    const itemData = this.data.data;
    const actorFlags = this.actor.data.flags.cnc || {};
    if ( !this.hasAttack || !itemData ) return;
  ...
  async roll({configureDialog=true, rollMode, createMessage=true}={}) {
    ...
    const requireSpellSlot = isSpell && (id.level > 0) && CONFIG.CNC.spellUpcastModes.includes(id.preparation.mode);
    ...
    if ( createMeasuredTemplate ) {
      const template = game.cnc.canvas.AbilityTemplate.fromItem(item);
  ...
  _getUsageUpdates({consumeQuantity, consumeRecharge, consumeResource, consumeSpellLevel, consumeUsage}) {
    ...
    if ( consumeRecharge ) {
      ...
      if ( recharge.charged === false ) {
        ui.notifications.warn(game.i18n.format("CNC.ItemNoUses", {name: this.name}));
        return false;
      }
    ...
    if ( consumeSpellLevel ) {
      ...
      if ( spells === 0 ) {
        const label = game.i18n.localize(consumeSpellLevel === "pact" ? "CNC.SpellProgPact" : `CNC.SpellLevel${id.level}`);
        ui.notifications.warn(game.i18n.format("CNC.SpellCastNoSlots", {name: this.name, level: label}));
        return false;
      }
  ...
  _handleConsumeResource(itemUpdates, actorUpdates, resourceUpdates) {
    ...
    // No consumed target
    const typeLabel = CONFIG.CNC.abilityConsumptionTypes[consume.type];
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("CNC.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( resource === undefined ) {
      ui.notifications.warn(game.i18n.format("CNC.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( remaining < 0 ) {
      ui.notifications.warn(game.i18n.format("CNC.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const html = await renderTemplate("systems/cnc/templates/chat/item-card.html", templateData);
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["cnc.itemData"] = this.data;
    }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      if ( data.attunement === CONFIG.CNC.attunementTypes.REQUIRED ) {
        props.push(CONFIG.CNC.attunements[CONFIG.CNC.attunementTypes.REQUIRED]);
      }
      props.push(
        game.i18n.localize(data.equipped ? "CNC.Equipped" : "CNC.Unequipped"),
        game.i18n.localize(data.proficient ? "CNC.Proficient" : "CNC.NotProficient")
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.CNC.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth.value ? game.i18n.localize("CNC.StealthDisadvantage") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.CNC.weaponTypes[data.weaponType]
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.CNC.consumableTypes[data.consumableType],
      `${data.uses.value}/${data.uses.max} ${game.i18n.localize("CNC.Charges")}`
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.CNC.abilities[data.ability] || null,
      CONFIG.CNC.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("CNC.ItemTypeLoot"),
      data.weight ? `${data.weight} ${game.i18n.localize("CNC.AbbreviationLbs")}` : null
    );
  }
  ...
  async rollAttack(options={}) {
    const itemData = this.data.data;
    const flags = this.actor.data.flags.cnc || {};
    if ( !this.hasAttack ) {
      throw new Error("You may not place an Attack Roll with this Item.");
    }
    let title = `${this.name} - ${game.i18n.localize("CNC.AttackRoll")}`;
    ...
    let rollConfig = {
      ...
      messageData: {
        "flags.cnc.roll": {type: "attack", itemId: this.id },
  ...
  rollDamage({critical=false, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const actorData = this.actor.data.data;
    const messageData = {
      "flags.cnc.roll": {type: "damage", itemId: this.id},
      speaker: ChatMessage.getSpeaker({actor: this.actor})
    };
    ...
    const actionFlavor = game.i18n.localize(itemData.actionType === "heal" ? "CNC.Healing" : "CNC.DamageRoll");
    ...
    if ( versatile && itemData.damage.versatile ) {
      ...
      messageData["flags.cnc.roll"].versatile = true;
    }
    ...
    if ( itemData.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("cnc", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula({spellLevel}={}) {
    ...
    const rollData = this.getRollData();
    if ( spellLevel ) rollData.item.level = spellLevel;
    const title = `${this.name} - ${game.i18n.localize("CNC.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.cnc.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("CNC.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "CNC.ItemRechargeSuccess" : "CNC.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("CNC.ToolCheck")}`;
    ...
    const rollConfig = mergeObject({
      ...
      chooseModifier: true,
      halflingLucky: this.actor.getFlag("cnc", "halflingLucky" ) || false,
      reliableTalent: (this.data.data.proficient >= 1) && this.actor.getFlag("cnc", "reliableTalent"),
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this.actor}),
        "flags.cnc.roll": {type: "tool", itemId: this.id }
      }
    }, options);
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("cnc", "itemData");
    const item = storedData ? new this(storedData, {parent: actor}) : actor.items.get(card.dataset.itemId);
    if ( !item ) {
      return ui.notifications.error(game.i18n.format("CNC.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}));
    }
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("CNC.ActionWarningNoToken"));
    return targets;
  }
  ...
  async createAdvancement(type, data={}, { showConfig=true }={}) {
    ...
    const Advancement = game.cnc.advancement.types[`${type}Advancement`];
    if ( !Advancement ) throw new Error(`${type}Advancement not found in game.cnc.advancement.types`);
   ...
   * @returns {Promise<ItemCnc>}  This item with the changes applied.
   */
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( changed.data.levels <= 0 ) {
      ui.notifications.warn(game.i18n.localize("CNC.MaxClassLevelMinimumWarn"));
      changed.data.levels = 1;
    }

    // Check to make sure the updated class level doesn't exceed level cap
    if ( changed.data.levels > CONFIG.CNC.maxLevel ) {
      ui.notifications.warn(game.i18n.format("CNC.MaxClassLevelExceededWarn", {max: CONFIG.CNC.maxLevel}));
      changed.data.levels = CONFIG.CNC.maxLevel;
    }
    ...
    if ( newCharacterLevel > CONFIG.CNC.maxLevel ) {
      ui.notifications.warn(game.i18n.format("CNC.MaxCharacterLevelExceededWarn",
        {max: CONFIG.CNC.maxLevel}));
      changed.data.levels -= newCharacterLevel - CONFIG.CNC.maxLevel;
    }
  }
  ...
  _onCreateOwnedEquipment(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const armorProf = CONFIG.CNC.armorProficienciesMap[this.data.data.armor?.type]; // Player characters check proficiency
  ...
  _onCreateOwnedWeapon(data, actorData, isNPC) {
    ...
    const weaponProf = CONFIG.CNC.weaponProficienciesMap[this.data.data.weaponType];
   ...
   * @param {ItemCnc} spell      The spell to be made into a scroll
   * @returns {ItemCnc}          The created scroll consumable item
   */
  static async createScrollFromSpell(spell) {

    // Get spell data
    const itemData = (spell instanceof ItemCnc) ? spell.toObject() : spell;
    const {
      actionType, description, source, activation, duration, target, range, damage, formula, save, level
    } = itemData.data;

    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.CNC.sourcePacks.ITEMS}.${CONFIG.CNC.spellScrollIds[level]}`;
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("CNC.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
