...
export class ScaleValueAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 60,
      icon: "systems/cnc/icons/svg/scale-value.svg",
      title: game.i18n.localize("CNC.AdvancementScaleValueTitle"),
      hint: game.i18n.localize("CNC.AdvancementScaleValueHint"),
      multiLevel: true,
   ...
   */
  static TYPES = {
    string: "CNC.AdvancementScaleValueTypeString",
    number: "CNC.AdvancementScaleValueTypeNumber",
    dice: "CNC.AdvancementScaleValueTypeDice",
    distance: "CNC.AdvancementScaleValueTypeDistance"
  };
  ...
  formatValue(level) {
    ...
    const value = this.valueForLevel(level);
    if ( value == null ) return null;
    return `${value.value} ${CONFIG.CNC.movementUnits[this.data.configuration.distance.units]}`;
  }
...
export class ScaleValueConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["cnc", "advancement", "scale-value", "two-column"],
      template: "systems/cnc/templates/advancement/scale-value-config.html",
      width: 540
    });
  }
  ...
  getData() {
    ...
    data.typeHint = game.i18n.localize(`CNC.AdvancementScaleValueTypeHint${config.type.capitalize()}`);
    data.types =
      Object.fromEntries(
    ...
    data.movementUnits = CONFIG.CNC.movementUnits;
    return data;
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    return Array.fromRange(CONFIG.CNC.maxLevel + 1).slice(1).reduce((obj, level) => {
  ...
  async _updateObject(event, formData) {
    ...
    if ( typeChange && (formData["data.configuration.type"] !== this.advancement.data.configuration.type) ) {
      ...
      Array.fromRange(CONFIG.CNC.maxLevel + 1).slice(1).forEach(l =>
...
export class ScaleValueFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/cnc/templates/advancement/scale-value-flow.html"
    });
  }
