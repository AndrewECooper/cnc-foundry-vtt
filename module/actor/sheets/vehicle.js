 ...
 * @type {ActorSheetCnc}
 */
export default class ActorSheetCncVehicle extends ActorSheetCnc {
  /** @inheritdoc */
  ...
  _computeEncumbrance(totalWeight, actorData) {
    ...
    totalWeight /= game.settings.get("cnc", "metricWeightUnits")
      ? CONFIG.CNC.encumbrance.vehicleWeightMultiplier.metric
      : CONFIG.CNC.encumbrance.vehicleWeightMultiplier.imperial;
  ...
  _prepareItems(data) {
    ...
    for (const item of data.items) {
      ...
      const isCargo = item.flags.cnc?.vehicleCargo === true;
      if ( isCargo ) {
   ...
   * @returns {Promise<ActorCnc>|null}  Actor after update if any changes were made.
   * @private
   */
   ...
   * @param {Event} event  Triggering event.
   * @returns {Promise<ItemCnc>}  Item with updates applied.
   * @private
   */
   ...
   * @param {Event} event  Triggering event.
   * @returns {Promise<ActorCnc|ItemCnc>}  Newly created actor or item depending on type created.
   * @private
   */
  _onItemCreate(event) {
    event.preventDefault();
   ...
   * @param {Event} event  Triggering event.
   * @returns {Promise<ActorCnc|ItemCnc>}  The deleted actor or item.
   * @private
   */
  _onItemDelete(event) {
    event.preventDefault();
  ...
  async _onDropItemCreate(itemData) {
    ...
    foundry.utils.setProperty(itemData, "flags.cnc.vehicleCargo", isCargo);
   ...
   * @param {Event} event  Triggering event.
   * @returns {Promise<ItemCnc>}  Item after the update is applied.
   * @private
   */
  _onHPChange(event) {
    event.preventDefault();
   ...
   * @param {Event} event  Triggering event.
   * @returns {Promise<ItemCnc>}  Item after the toggling is applied.
   * @private
   */
  _onToggleItem(event) {
    event.preventDefault();
