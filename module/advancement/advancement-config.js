...
export class AdvancementConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["cnc", "advancement", "dialog"],
      template: "systems/cnc/templates/advancement/advancement-config.html",
      width: 400,
      height: "auto",
      submitOnChange: true,
      closeOnSubmit: false
  ...
  get title() {
    ...
    return `${game.i18n.format("CNC.AdvancementConfigureTitle", { item: this.item.name })}: ${type}`;
  }
  ...
  /** @inheritdoc */
  getData() {
    const levels = Object.fromEntries(Array.fromRange(CONFIG.CNC.maxLevel + 1).map(l => [l, l]));
    if ( ["class", "subclass"].includes(this.item.type) ) delete levels[0];
    else levels[0] = game.i18n.localize("CNC.AdvancementLevelAnyHeader");
