...
export default class ActorSkillConfig extends DocumentSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["cnc"],
      template: "systems/cnc/templates/apps/skill-config.html",
      width: 500,
      height: "auto"
    });
  }
  ...
  /** @inheritdoc */
  get title() {
    return `${game.i18n.format("CNC.SkillConfigureTitle", {skill: CONFIG.CNC.skills[this._skillId]})}: ${this.document.name}`;
  }
  ...
  getData(options) {
    return {
      ...
      proficiencyLevels: CONFIG.CNC.proficiencyLevels,
  ...
  _updateObject(event, formData) {
    ...
    if ( !passiveRoll.isDeterministic ) {
      const message = game.i18n.format("CNC.FormulaCannotContainDiceError", {
        name: game.i18n.localize("CNC.SkillBonusPassive")
      });
