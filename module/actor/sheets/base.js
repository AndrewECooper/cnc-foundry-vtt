...
import ActiveEffectCnc from "../../active-effect.js";
...
export default class ActorSheetCnc extends ActorSheet {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      width: 720,
      height: Math.max(680, 237 + (Object.keys(CONFIG.CNC.abilities).length * 70))
    });
  }
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/cnc/templates/actors/limited-sheet.html";
    return `systems/cnc/templates/actors/${this.actor.data.type}-sheet.html`;
  }
  ...
  getData(options) {
    ...
    // Currency Labels
    data.labels.currencies = Object.entries(CONFIG.CNC.currencies).reduce((obj, [k, c]) => {
      obj[k] = c.label;
      return obj;
    }, {});
    ...
    // Proficiency
    if ( game.settings.get("cnc", "proficiencyModifier") === "dice" ) {
    ...
    for ( let [a, abl] of Object.entries(actorData.data.abilities)) {
      ...
      abl.hover = CONFIG.CNC.proficiencyLevels[abl.proficient];
      abl.label = CONFIG.CNC.abilities[a];
    ...
    if ( actorData.data.skills ) {
      for (let [s, skl] of Object.entries(actorData.data.skills)) {
        skl.ability = CONFIG.CNC.abilityAbbreviations[skl.ability];
        skl.icon = this._getProficiencyIcon(skl.value);
        skl.hover = CONFIG.CNC.proficiencyLevels[skl.value];
        skl.label = CONFIG.CNC.skills[s];
    ...
    data.effects = ActiveEffectCnc.prepareActiveEffectCategories(this.actor.effects);
  ...
  _getMovementSpeed(actorData, largestPrimary=false) {
    ...
    let speeds = [
      [movement.burrow, `${game.i18n.localize("CNC.MovementBurrow")} ${movement.burrow}`],
      [movement.climb, `${game.i18n.localize("CNC.MovementClimb")} ${movement.climb}`],
      [movement.fly, `${game.i18n.localize("CNC.MovementFly")} ${movement.fly}${movement.hover ? ` (${game.i18n.localize("CNC.MovementHover")})` : ""}`],
      [movement.swim, `${game.i18n.localize("CNC.MovementSwim")} ${movement.swim}`]
    ];
    if ( largestPrimary ) {
      speeds.push([movement.walk, `${game.i18n.localize("CNC.MovementWalk")} ${movement.walk}`]);
    }
  ...
  _getSenses(actorData) {
    ...
    const tags = {};
    for ( let [k, label] of Object.entries(CONFIG.CNC.senses) ) {
      const v = senses[k] ?? 0;
      if ( v === 0 ) continue;
  ...
  _prepareArmorClassAttribution(data) {
    ...
    const cfg = CONFIG.CNC.armorClasses[ac.calc];
    const attribution = [];
    ...
    switch ( ac.calc ) {
      ...
      case "flat":
        return [{
          label: game.i18n.localize("CNC.ArmorClassFlat"),
      ...
      case "natural":
        attribution.push({
          label: game.i18n.localize("CNC.ArmorClassNatural"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: ac.flat
        });
        break;
      ...
      default:
        ...
        let label = game.i18n.localize("CNC.PropertyBase");
        if ( armorInFormula ) label = this.actor.armor?.name ?? game.i18n.localize("CNC.ArmorClassUnarmored");
        attribution.unshift({
          label,
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base
    ...
    if ( ac.shield !== 0 ) attribution.push({
      label: this.actor.shield?.name ?? game.i18n.localize("CNC.EquipmentShield"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.shield
    });
  ...
  _prepareTraits(traits) {
    const map = {
      dr: CONFIG.CNC.damageResistanceTypes,
      di: CONFIG.CNC.damageResistanceTypes,
      dv: CONFIG.CNC.damageResistanceTypes,
      ci: CONFIG.CNC.conditionTypes,
      languages: CONFIG.CNC.languages
    };
    ...
    for ( const t of ["armor", "weapon", "tool"] ) {
      ...
      if ( !trait ) continue;
      ActorCnc.prepareProficiencies(trait, t);
      trait.cssClass = !isObjectEmpty(trait.selected) ? "" : "inactive";
    }
  }
  ...
  _prepareSpellbook(data, spells) {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.CNC.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.CNC.spellLevels[lvl], levels[sl]);
      }
    }

    // Pact magic users have cantrips and a pact magic section
    if ( levels.pact && levels.pact.max ) {
      if ( !spellbook["0"] ) registerSection("spell0", 0, CONFIG.CNC.spellLevels[0]);
      const l = levels.pact;
      const config = CONFIG.CNC.spellPreparationModes.pact;
      const level = game.i18n.localize(`CNC.SpellLevel${levels.pact.level}`);
    ...
    spells.forEach(spell => {
      ...
      if ( mode in sections ) {
        ...
        if ( !spellbook[s] ) {
          const l = levels[mode] || {};
          const config = CONFIG.CNC.spellPreparationModes[mode];
      ...
      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
        ...
        registerSection(sl, s, CONFIG.CNC.spellLevels[s], {levels: levels[sl]});
      }
   ...
   * @param {number} level  A proficiency mode defined in `CONFIG.CNC.proficiencyLevels`.
  ...
  activateListeners(html) {
    ...
    if ( this.isEditable ) {
      ...
      html.find(".effect-control").click(ev => ActiveEffectCnc.onManageActiveEffect(ev, this.actor));
    }
  ...
  async _onDropActor(event, data) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get("cnc", "allowPolymorphing"));
    ...
    const rememberOptions = html => {
      ...
      const settings = mergeObject(game.settings.get("cnc", "polymorphSettings") || {}, options);
      game.settings.set("cnc", "polymorphSettings", settings);
      return settings;
    };

    // Create and render the Dialog
    return new Dialog({
      title: game.i18n.localize("CNC.PolymorphPromptTitle"),
      content: {
        options: game.settings.get("cnc", "polymorphSettings"),
        i18n: CONFIG.CNC.polymorphSettings,
      ...
      buttons: {
        accept: {
          ...
          label: game.i18n.localize("CNC.PolymorphAcceptSettings"),
          callback: html => this.actor.transformInto(sourceActor, rememberOptions(html))
        },
        wildshape: {
          icon: '<i class="fas fa-paw"></i>',
          label: game.i18n.localize("CNC.PolymorphWildShape"),
        ...
        polymorph: {
          ...
          label: game.i18n.localize("CNC.Polymorph"),
          callback: html => this.actor.transformInto(sourceActor, {
            transformTokens: rememberOptions(html).transformTokens
          })
        },
      ...
      classes: ["dialog", "cnc"],
      width: 600,
      template: "systems/cnc/templates/apps/polymorph-prompt.html"
    }).render(true);
  }
  ...
  async _onDropItemCreate(itemData) {
    ...
    if ( this.constructor.unsupportedItemTypes.has(itemData.type) ) {
      return ui.notifications.warn(game.i18n.format("CNC.ActorWarningInvalidItem", {
    ...
    if ( (itemData.type === "spell") && (this._tabs[0].active === "inventory") ) {
      const scroll = await ItemCnc.createScrollFromSpell(itemData);
    ...
    if ( itemData.data.advancement?.length && !game.settings.get("cnc", "disableAdvancements") ) {
  ...
  _onDropResetData(itemData) {
    ...
    itemData.data.attunement = Math.min(itemData.data.attunement, CONFIG.CNC.attunementTypes.REQUIRED);
  }
   ...
   * @returns {Promise<ItemCnc>|null}  If a duplicate was found, returns the adjusted item stack.
   */
   ...
   * @returns {Promise<ItemCnc>}  Updated item.
   * @private
   */
   ...
   * @param {Event} event          The originating click event.
   * @returns {Promise<ItemCnc[]>}  The newly created item.
   * @private
   */
  _onItemCreate(event) {
    event.preventDefault();
    ...
    if ( type === "class" && (this.actor.data.data.details.level + 1 > CONFIG.CNC.maxLevel) ) {
      return ui.notifications.error(game.i18n.format("CNC.MaxCharacterLevelExceededWarn",
        {max: CONFIG.CNC.maxLevel}));
    }

    const itemData = {
      name: game.i18n.format("CNC.ItemNew", {type: game.i18n.localize(`CNC.ItemType${type.capitalize()}`)}),
      type: type,
   ...
   * @param {Event} event    The originating click event.
   * @returns {ItemSheetCnc}  The rendered item sheet.
   * @private
   */
  _onItemEdit(event) {
    event.preventDefault();
   ...
   * @param {Event} event  The originating click event.
   * @returns {Promise<ItemCnc|AdvancementManager>|undefined}  The deleted item if something was deleted or the
  ...
  async _onItemDelete(event) {
    ...
    if ( item.hasAdvancement && !game.settings.get("cnc", "disableAdvancements") ) {
   ...
   * @param {Event} event         The originating click event.
   * @returns {Promise<ActorCnc>}  Updated actor instance.
   * @private
   */
   ...
   * @returns {ActorSheetCnc}  This actor sheet with toggled filters.
   * @private
   */
  ...
  _onTraitSelector(event) {
    ...
    const a = event.currentTarget;
    const label = a.parentElement.querySelector("label");
    const choices = CONFIG.CNC[a.dataset.options];
  ...
  _getHeaderButtons() {
    ...
    if ( this.actor.isPolymorphed ) {
      buttons.unshift({
        label: "CNC.PolymorphRestoreTransformation",
