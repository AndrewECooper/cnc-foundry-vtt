...
export class ItemGrantAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 40,
      icon: "systems/cnc/icons/svg/item-grant.svg",
      title: game.i18n.localize("CNC.AdvancementItemGrantTitle"),
      hint: game.i18n.localize("CNC.AdvancementItemGrantHint"),
      apps: {
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( !this.data.value.added || configMode ) {
      return this.data.configuration.items.reduce((html, uuid) => html + game.cnc.utils._linkForUuid(uuid), "");
    }
  ...
  async apply(level, data, retainedData={}) {
    ...
    for ( const [uuid, selected] of Object.entries(data) ) {
      ...
      item.data.update({
        ...
        "flags.cnc.sourceId": uuid,
        "flags.cnc.advancementOrigin": `${this.item.id}.${this.id}`
      });
  ...
  restore(level, data) {
    ...
    for ( const item of data.items ) {
      ...
      updates[item._id] = item.flags.cnc.sourceId;
    }
    this.updateSource({"value.added": updates});
  }
...
export class ItemGrantConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/cnc/templates/advancement/item-grant-config.html"
    });
  }
   ...
   * @returns {Promise<ItemCnc>}  The promise for the updated parent Item which resolves after the application re-renders
   * @private
   */
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      return ui.notifications.warn(game.i18n.localize("CNC.AdvancementItemGrantRecursiveWarning"));
    }

    // Abort if this uuid exists already
    if ( existingItems.includes(item.uuid) ) {
      return ui.notifications.warn(game.i18n.localize("CNC.AdvancementItemGrantDuplicateWarning"));
    }
...
export class ItemGrantFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/cnc/templates/advancement/item-grant-flow.html"
    });
  }
  ...
  async getData() {
    ...
    const added = this.retainedData?.items.map(i => foundry.utils.getProperty(i, "flags.cnc.sourceId"))
  ...
  async _updateObject(event, formData) {
    const retainedData = this.retainedData?.items.reduce((obj, i) => {
      obj[foundry.utils.getProperty(i, "flags.cnc.sourceId")] = i;
      return obj;
    }, {});
