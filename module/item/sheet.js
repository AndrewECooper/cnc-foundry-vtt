 ...
 * @extends {ItemSheet}
 */
export default class ItemSheetCnc extends ItemSheet {
  ...
  async getData(options) {
    ...
    data.effects = ActiveEffectCnc.prepareActiveEffectCategories(this.item.effects);
   ...
   * @param {ItemCnc} item  The item for which the advancement is being prepared.
  ...
  _getItemConsumptionTargets(item) {
    ...
    // Hit Dice
      ...
      return {
        smallest: game.i18n.localize("CNC.ConsumeHitDiceSmallest"),
        ...CONFIG.CNC.hitDieTypes.reduce((obj, hd) => { obj[hd] = hd; return obj; }, {}),
        largest: game.i18n.localize("CNC.ConsumeHitDiceLargest")
      };
    }
  ...
  _getItemStatus(item) {
    switch ( item.type ) {
      case "class":
        return game.i18n.format("CNC.LevelCount", {ordinal: item.data.levels.ordinalString()});
      case "equipment":
      case "weapon":
  ...
  _getSubmitData(updateData={}) {
    ...
    if ( data.data?.uses?.max ) {
      ...
      if ( !maxRoll.isDeterministic ) {
        ...
        return ui.notifications.error(game.i18n.format("CNC.FormulaCannotContainDiceError", {
          name: game.i18n.localize("CNC.LimitedUses")
        }));
      }
    }
    ...
    if ( data.data.identifier ) {
      ...
      if ( !match ) {
        ...
        return ui.notifications.error(game.i18n.localize("CNC.IdentifierError"));
      }
    }
  ...
  activateListeners(html) {
    ...
    if ( this.isEditable ) {
      ...
      html.find(".effect-control").click(ev => {
        ...
        ActiveEffectCnc.onManageActiveEffect(ev, this.item);
      });
   ...
   * @returns {Promise<ItemCnc>|null}  Item with updates applied.
   * @private
   */
  ...
  _onConfigureTraits(event) {
    ...
    switch (a.dataset.options) {
      case "saves":
        options.choices = CONFIG.CNC.abilities;
        options.valueKey = null;
        break;
      case "skills.choices":
        options.choices = CONFIG.CNC.skills;
        options.valueKey = null;
        break;
      case "skills":
        const skills = this.item.data.data.skills;
        const choiceSet = skills.choices?.length ? skills.choices : Object.keys(CONFIG.CNC.skills);
        options.choices =
          Object.fromEntries(Object.entries(CONFIG.CNC.skills).filter(([skill]) => choiceSet.includes(skill)));
  ...
  _onAdvancementAction(event) {
    ...
    if ( cl.contains("item-add") ) return game.cnc.advancement.AdvancementSelection.createDialog(this.item);
