...
export const migrateWorld = async function() {
  ...
  ui.notifications.info(game.i18n.format("MIGRATION.CncBegin", {version}), {permanent: true});
  ...
  for ( let a of game.actors ) {
    try {
      ...
      err.message = `Failed cnc system migration for Actor ${a.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let i of game.items ) {
    try {
      ...
      err.message = `Failed cnc system migration for Item ${i.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const m of game.macros ) {
    try {
      ...
      err.message = `Failed cnc system migration for Macro ${m.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes ) {
    try {
      ...
      err.message = `Failed cnc system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  game.settings.set("cnc", "systemMigrationVersion", game.system.data.version);
  ui.notifications.info(game.i18n.format("MIGRATION.CncComplete", {version}), {permanent: true});
};
...
export const migrateCompendium = async function(pack) {
  ...
  for ( let doc of documents ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed cnc system migration for document ${doc.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
export const migrateArmorClass = async function(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.CNC.armorTypes));
...
function cleanActorData(actorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.CNC.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if ( actorData.flags.cnc ) {
    actorData.flags.cnc = filterObject(actorData.flags.cnc, allowedFlags);
  }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/cnc/json/icon-migration.json");
    const spellIcons = await fetch("systems/cnc/json/spell-icon-migration.json");
...
function _migrateActorType(actor, updateData) {
  ...
  if ( match ) {
    ...
    const typeMatch = Object.entries(CONFIG.CNC.creatureTypes).find(([k, v]) => {
      return (typeLc === k)
    ...
    // Match a swarm
    const isNamedSwarm = actor.name.startsWith(game.i18n.localize("CNC.CreatureSwarm"));
    if ( match.groups.size || isNamedSwarm ) {
      const sizeLc = match.groups.size ? match.groups.size.trim().toLowerCase() : "tiny";
      const sizeMatch = Object.entries(CONFIG.CNC.actorSizes).find(([k, v]) => {
...
const TOKEN_IMAGE_RENAME = {
  ...
  "systems/cnc/tokens/beast/ScorpionSand.png": "systems/cnc/tokens/beast/Scorpion.webp",
  "systems/cnc/tokens/beast/BaboonBlack.png": "systems/cnc/tokens/beast/Baboon.webp",
  "systems/cnc/tokens/humanoid/BanditRedM.png": "systems/cnc/tokens/humanoid/Bandit.webp",
  "systems/cnc/tokens/humanoid/GuardBlueM.png": "systems/cnc/tokens/humanoid/Guard.webp",
  "systems/cnc/tokens/humanoid/HumanBrownM.png": "systems/cnc/tokens/humanoid/Commoner.webp",
  "systems/cnc/tokens/humanoid/NobleSwordM.png": "systems/cnc/tokens/humanoid/Noble.webp",
  "systems/cnc/tokens/humanoid/MerfolkBlue.png": "systems/cnc/tokens/humanoid/Merfolk.webp",
  "systems/cnc/tokens/humanoid/TribalWarriorM.png": "systems/cnc/tokens/humanoid/TribalWarrior.webp",
  "systems/cnc/tokens/devil/Lemure.png": "systems/cnc/tokens/fiend/Lemure.webp",
  "systems/cnc/tokens/humanoid/Satyr.png": "systems/cnc/tokens/fey/Satyr.webp",
  "systems/cnc/tokens/beast/WinterWolf.png": "systems/cnc/tokens/monstrosity/WinterWolf.webp"
};
...
const TOKEN_IMAGE_RESCALE = {
  ...
  "systems/cnc/tokens/beast/GiantElk.png": 1.5,
  "systems/cnc/tokens/monstrosity/Bulette.png": 1.5,
  "systems/cnc/tokens/beast/Wolf.png": 1.5,
  "systems/cnc/tokens/beast/Panther.png": 1.5,
  "systems/cnc/tokens/beast/Elk.png": 1.5,
  "systems/cnc/tokens/beast/AxeBeak.png": 1.5,
  "systems/cnc/tokens/beast/GiantVulture.png": 1.5,
  "systems/cnc/tokens/beast/GiantSpider.png": 1.5,
  "systems/cnc/tokens/beast/DireWolf.png": 1.5,
  "systems/cnc/tokens/monstrosity/DeathDog.png": 1.5,
  "systems/cnc/tokens/devil/Lemure.png": 1.5,
  "systems/cnc/tokens/beast/Deer.png": 1.1,
  "systems/cnc/tokens/beast/GiantWeasel.png": 1.5,
  "systems/cnc/tokens/beast/Camel.png": 1.2,
  "systems/cnc/tokens/beast/BloodHawk.png": 1.5
};
...
function _migrateTokenImage(actorData, updateData) {
  ["img", "token.img"].forEach(prop => {
    ...
    if ( img === "systems/cnc/tokens/humanoid/HumanBrownM.webp" ) {
      updateData[prop] = "systems/cnc/tokens/humanoid/Commoner.webp";
      return updateData;
    }
    if ( !img?.startsWith("systems/cnc/tokens/") || img?.endsWith(".webp") ) return;
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["data.attunement"] = CONFIG.CNC.attunementTypes.NONE;
...
function _migrateItemRarity(item, updateData) {
  ...
  const rarity = Object.keys(CONFIG.CNC.itemRarity).find(key =>
    (CONFIG.CNC.itemRarity[key].toLowerCase() === item.data.rarity.toLowerCase()) || (key === item.data.rarity)
  );
...
function _migrateItemIcon(item, updateData, {iconMap}={}) {
  if ( iconMap && item.img?.startsWith("systems/cnc/icons/") ) {
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flagsCnc = flags.cnc || null;
    return flagsCnc ? {cnc: flagsCnc} : {};
  };
  await pack.configure({locked: false});
